    ...
    },
    {
      ...
      events: [
        ...
        { name: 'Sangeet', time: '19:00', end: '23:30', icon: 'music_note',
          note: 'Dhol, dance floor and a night that refuses to end.', dress: 'Indo Western' }
      ]
    },
    {
    ...
    const rasams = day.events.map((ev, ei) => `
      <li class="rasam">
        ...
        <div class="rasam-foot">
          ${ev.dress ? `<span class="dress">${ev.dress}</span>` : ''}
...
function startMusic() {
  ...
  window.onYouTubeIframeAPIReady = () => {
    ytPlayer = new YT.Player('ytHost', {
      videoId: cfg.videoId,
      playerVars: { autoplay: 1, controls: 0, start: cfg.startAt, playsinline: 1, rel: 0, modestbranding: 1 },
      events: {
        onReady: (e) => {
        ...
        /* loop back to the same point rather than restarting from zero */
        onStateChange: (e) => {
  ...
  tag.onerror = () => { /* offline or blocked — the page simply stays quiet */ };
...
function toggleMusic() {
  ...
  if (muted) { ytPlayer.unMute(); btn.classList.remove('off'); icon.textContent = 'volume_up'; }
  else { ytPlayer.mute(); btn.classList.add('off'); icon.textContent = 'volume_off'; }
}
